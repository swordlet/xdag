...
xmrig::VirtualMemory::VirtualMemory(size_t size, bool hugePages, bool oneGbPages, bool usePool, uint32_t node, size_t alignSize) :
    ...
    m_node(node)
{
		printf("new virtual memory size %u, hugePages %d, onGbPages %d, usePool %d node %u, alignSize %u \n",
				size,hugePages,oneGbPages,usePool,node,alignSize);

    if (usePool) {
	      printf("use pool \n");
        std::lock_guard<std::mutex> lock(mutex);
        if (hugePages && !pool->isHugePages(node) && allocateLargePagesMemory()) {
        	  printf("use pool hugePages alloc large pages memory\n");
            return;
        }

        m_scratchpad = pool->get(m_size, node);
        if (m_scratchpad) {
	          printf("scrath pad set flag hugepages external\n");
    ...
    if (oneGbPages && allocateOneGbPagesMemory()) {
	      printf("one gb pages alloc one gb pages memory\n");
        return;
    }

    if (hugePages && allocateLargePagesMemory()) {
	      printf("huge pages alloc large pages memory\n");
        return;
    }
...
#   ifdef XMRIG_FEATURE_HWLOC
    if (Cpu::info()->nodes() > 1) {
    	  LOG_INFO("alloc NUMA Memory Pool poolSize %u, Cpu::info nodes %u, hugePages %d",poolSize,Cpu::info()->nodes(),hugePages);
        pool = new NUMAMemoryPool(align(poolSize, Cpu::info()->nodes()), hugePages);
    } else
#   endif
    {
	      printf("alloc Memory Pool poolSize %u, hugePages %d\n",poolSize,hugePages);
