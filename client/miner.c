		...
		if(p.revents & POLLIN) {
			...
			if(ndata == maxndata) {
				...
				if(!memcmp(last->data, hash, sizeof(xdag_hashlow_t))) {
					...
					xdag_info("### receive rx key %016llx%016llx%016llx%016llx",data[1].data[3],data[1].data[2],data[1].data[1],data[1].data[0]);
...
//					          task->minhash.data[0],task->minhash.data[1],task->minhash.data[2],task->minhash.data[3]);
		...
		if(p.revents & POLLOUT) {
            ...
            if(task->minhash.amount + 1 != 0) { // min hash is not  0xffffffffff....
                ...
                xdag_info("Sent lastfield data %016llx%016llx%016llx%016llx", d[3], d[2], d[1], d[0]);
                xdag_info("Share : %016llx%016llx%016llx%016llx t=%llx res=%d",
                          h[3], h[2], h[1], h[0], task->task_time << 16 | 0xffff, res);
