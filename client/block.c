...
static void log_block(const char *mess, xdag_hash_t h, xtime_t t, uint64_t pos) {
	...
	if (g_xdag_state != XDAG_STATE_LOAD) {
		xdag_info("%s tid=%lu height=%lu: %016llx%016llx%016llx%016llx t=%llx pos=%llx", mess, pthread_self(),
		          g_xdag_stats.nmain,
...
//        }
		...
		for (memcpy(&blockRef, nodeBlock, sizeof(struct block_internal));
			...
			if ((xd_rsdb_get_bi(blockRef.link[blockRef.max_diff_link], &br_max) ||
...
//        g_xdag_stats.difficulty = tmpNodeBlock.difficulty;
		...
		if (pretop_main_chain) {
			...
			xd_rsdb_put_setting(SETTING_PRE_TOP_MAIN_HASH, (const char *) g_pre_top_main_chain_hash,
			                    sizeof(g_pre_top_main_chain_hash));
		}
		if (top_main_chain) {
	...
	log_block((tmpNodeBlock.flags & BI_OURS ? "Good +" : "Good  "), tmpNodeBlock.hash, tmpNodeBlock.time,
	          tmpNodeBlock.storage_pos);
...
void append_block_info(struct block_internal *bi) {
	...
	sprintf(message,
	        ...
	        xdag_diff_args(bi->difficulty), remark, address, pramount(bi->amount)
	);
...
static void print_block(struct block_internal *block, int print_only_addresses, FILE *out) {
	...
	if (print_only_addresses) {
		...
		fprintf(out, "%08llu   %s   %s   %-8s  %-32s\n", block->height, address, time_buf,
		        xdag_get_block_state_info(block->flags), remark);
	}
}
	...
	if (!xd_rsdb_get_bi(hash, &b)) {
		...
		for (int i = 0; i < bi->nlinks; i++) {
			...
			if (bi->linkamount[i] && !xd_rsdb_get_bi(bi->link[i], &b)) {
				if (callback(data, 1 << i & bi->in_mask, bi->flags, bi->hash, bi->linkamount[i], bi->time,
				             (const char *) remark)) {
					return i;
				}
			}
